...
' Start the queries
       ...
       fsql = "select * from CVSUser where 1 =1  "

       if findnum <> "" then

       fsql = fsql & "and (UserName like '%" & findnum &"%') "

       end if

       fsql = fsql & " order by UserName desc"
...
%>
   <tr>
<td >
<input type="checkbox" name="mid" value="<% = frs("UserName") %>">
</td>
<td  height="28">
<a href="user2.asp?UserName=<%=frs("UserName")%>"><% = frs("UserName") %></a>
</td>
...
if (k==0)
...
else if (k==1)
 {
  ...
  if (confirm(msg)==true)
   {
    document.fm1.whatdo.value="del_usr";
    document.fm1.submit();
   }
 }
...
<input type="button" value="    New User    " onClick="dosubmit();" class="common">
